#!/bin/bash
sed -i "s/const \[activeTab, setActiveTab\] = useState<'list' | 'board' | 'timeline' | 'monitoring'>('list');/const \[activeTab, setActiveTab\] = useState<'list' | 'board' | 'timeline' | 'monitoring' | 'documents'>('list');/g" src/pages/ProjectDetail.tsx

sed -i '/<div className="flex space-x-4 px-6 overflow-x-auto">/a \
            <button \
            ...
            </button>' src/pages/ProjectDetail.tsx

cat << 'INNER_EOF' > inject_docs.txt
            {activeTab === 'documents' && (
...
# Find line of {activeTab === 'monitoring' && (
MONITORING_END=$(grep -n "{activeTab === 'monitoring' && (" src/pages/ProjectDetail.tsx | head -n 1 | cut -d: -f1)

# We want to insert the doc block RIGHT BEFORE the last closing tags of the main content.
sed -i -e '/<\/div>$/,/<\/div>$/!b' -e '/{activeTab === '"'"'monitoring'"'"' && (/i\
'"$(cat inject_docs.txt | sed 's/$/\\/')"'' src/pages/ProjectDetail.tsx
# The above sed command might be tricky. Let's do it safer.
